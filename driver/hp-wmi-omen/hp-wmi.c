...
#if __has_include(<linux/array_size.h>)
#include <linux/array_size.h>
#endif
#include <linux/bits.h>
#if __has_include(<linux/cleanup.h>)
#include <linux/cleanup.h>
#else
static inline void _hp_mutex_cleanup(struct mutex **m)
{
	if (*m)
		mutex_unlock(*m);
}
#define _hp_var2(pfx, line) pfx##line
#define _hp_var(pfx, line) _hp_var2(pfx, line)
#define guard(name) _guard_##name
#define _guard_mutex(m)                                                                                                \
	struct mutex *_hp_var(_m_, __LINE__) __attribute__((__cleanup__(_hp_mutex_cleanup))) = (mutex_lock(m), (m))
#define scoped_guard(name, lock) for (int _i = (mutex_lock(lock), 0); !_i; _i = 1, mutex_unlock(lock))
#endif
...
static int hp_wmi_compat_get(struct platform_profile_handler *pph, enum platform_profile_option *profile)
{
	return hp_wmi_compat_ops->profile_get(NULL, profile);
}

static int hp_wmi_compat_set(struct platform_profile_handler *pph, enum platform_profile_option profile)
{
...
static struct device *devm_platform_profile_register(struct device *dev, const char *name, void *drvdata,
...
static int hp_wmi_compat_get_int(struct platform_profile_handler *pph, enum platform_profile_option *profile)
{
	return hp_wmi_compat_ops->profile_get(NULL, profile);
}

static int hp_wmi_compat_set_int(struct platform_profile_handler *pph, enum platform_profile_option profile)
{
	return hp_wmi_compat_ops->profile_set(NULL, profile);
}
...
static const char *const omen_thermal_profile_boards[] = {
	"84DA", "84DB", "84DC", "8572", "8573", "8574", "8575", "8600", "8601", "8602", "8603", "8604",
	"8605", "8606", "8607", "860A", "8746", "8747", "8748", "8749", "874A", "8786", "8787", "8788",
	"878A", "878B", "878C", "87B5", "886B", "886C", "88C8", "88CB", "88D1", "88D2", "88F4", "88F5",
	"88F6", "88F7", "88FD", "88FE", "88FF", "8900", "8901", "8902", "8912", "8917", "8918", "8949",
	"894A", "89EB", "8A15", "8A42", "8A43", "8BAD", "8C58", "8E41", "8BA9", "8BCA", "8D2F",
};
...
static const struct key_entry hp_wmi_keymap[] = {{KE_KEY, 0x02, {KEY_BRIGHTNESSUP}},
						 ...
						 }, /* FnLock off */
						 {
							 KE_IGNORE,
							 0x30021aa,
						 }, /* kbd backlight: level 2 -> off */
						 {
							 KE_IGNORE,
							 0x33221aa,
						 }, /* kbd backlight: off -> level 1 */
						 {
							 KE_IGNORE,
							 0x36421aa,
						 }, /* kbd backlight: level 1 -> level 2*/
						 {KE_END, 0}};
...
static int hp_wmi_get_fan_speed(int fan)
{
	...
	int ret = hp_wmi_perform_query(HPWMI_FAN_SPEED_GET_QUERY, HPWMI_GM, &fan_data, sizeof(char), sizeof(fan_data));
...
static int hp_wmi_get_fan_speed_victus_s(int fan)
{
	...
	ret = hp_wmi_perform_query(HPWMI_VICTUS_S_FAN_SPEED_GET_QUERY, HPWMI_GM, &fan_data, sizeof(u8),
				   sizeof(fan_data));
	if (ret != 0)
		return -EINVAL;
...
static int hp_wmi_get_tablet_mode(void)
{
	...
	tablet_found = match_string(tablet_chassis_types, ARRAY_SIZE(tablet_chassis_types), chassis_type) >= 0;
	if (!tablet_found)
		return -ENODEV;

	ret = hp_wmi_perform_query(HPWMI_SYSTEM_DEVICE_MODE, HPWMI_READ, system_device_mode,
...
static bool is_omen_thermal_profile(void)
{
	...
	return match_string(omen_thermal_profile_boards, ARRAY_SIZE(omen_thermal_profile_boards), board_name) >= 0;
}
...
static int omen_get_thermal_policy_version(void)
{
	...
	if (board_name) {
		int matches = match_string(omen_thermal_profile_force_v0_boards,
					   ARRAY_SIZE(omen_thermal_profile_force_v0_boards), board_name);
		if (matches >= 0)
			return 0;
	}
...
static int hp_wmi_fan_speed_max_set(int enabled)
{
	...
	ret = hp_wmi_perform_query(HPWMI_FAN_SPEED_MAX_SET_QUERY, HPWMI_GM, &enabled, sizeof(enabled), 0);
...
static int hp_wmi_fan_speed_set(struct hp_wmi_hwmon_priv *priv)
{
	...
	if (priv->cpu_pwm == HP_FAN_SPEED_AUTOMATIC)
		...
		fan_speed[CPU_FAN] = pwm_to_rpm(priv->cpu_pwm, priv->cpu_max_rpm);

	if (priv->gpu_pwm == HP_FAN_SPEED_AUTOMATIC)
		fan_speed[GPU_FAN] = HP_FAN_SPEED_AUTOMATIC;
	else
		fan_speed[GPU_FAN] = pwm_to_rpm(priv->gpu_pwm, priv->gpu_max_rpm);
	...
	if (ret < 0)
		return ret;
	ret = hp_wmi_perform_query(HPWMI_VICTUS_S_FAN_SPEED_SET_QUERY, HPWMI_GM, &fan_speed, sizeof(fan_speed), 0);
...
{
	int state = 0;
	int ret = hp_wmi_perform_query(HPWMI_FEATURE_QUERY, HPWMI_READ, &state, zero_if_sup(state), sizeof(state));
	if (!ret)
		return 1;
...
{
	u8 state[128];
	int ret = hp_wmi_perform_query(HPWMI_FEATURE2_QUERY, HPWMI_READ, &state, zero_if_sup(state), sizeof(state));
	if (!ret)
		return 1;
...
{
	int value = 0x6e;
	int ret = hp_wmi_perform_query(HPWMI_BIOS_QUERY, HPWMI_WRITE, &value, sizeof(value), 0);
...
static int hp_wmi_set_block(void *data, bool blocked)
{
	...
	ret = hp_wmi_perform_query(HPWMI_WIRELESS_QUERY, HPWMI_WRITE, &query, sizeof(query), 0);
...
static int hp_wmi_rfkill2_set_block(void *data, bool blocked)
{
	...
	ret = hp_wmi_perform_query(HPWMI_WIRELESS2_QUERY, HPWMI_WRITE, buffer, sizeof(buffer), 0);
...
static int hp_wmi_rfkill2_refresh(void)
{
	...
	err = hp_wmi_perform_query(HPWMI_WIRELESS2_QUERY, HPWMI_READ, &state, zero_if_sup(state), sizeof(state));
	if (err)
		return err;
	...
	for (i = 0; i < rfkill2_count; i++) {
		...
		rfkill_set_states(rfkill2[i].rfkill, IS_SWBLOCKED(devstate->power), IS_HWBLOCKED(devstate->power));
	}
...
static int hp_wmi_get_mux_supported_modes(u8 *supported)
{
	...
	ret = hp_wmi_perform_query(HPWMI_GET_SYSTEM_DESIGN_DATA, HPWMI_GM, buffer, zero_if_sup(req_packet),
				   sizeof(buffer));
	if (ret == 0) {
		*supported = buffer[7];
		return 0;
	 ...
	 */
	ret = hp_wmi_perform_query(HPWMI_GRAPHICS_MUX_QUERY, HPWMI_READ, legacy_buffer, sizeof(legacy_buffer), 0);
	if (ret < 0)
		return ret;
	if (ret > 0)
		return -EINVAL;
...
static int hp_wmi_get_mux_mode(u8 *mode)
{
	...
	ret = hp_wmi_perform_query(HPWMI_GRAPHICS_MUX_QUERY, HPWMI_READ, buffer, sizeof(buffer), sizeof(buffer));
	if (ret < 0)
		return ret;
	if (ret > 0)
		return -EINVAL;
...
static int hp_wmi_set_mux_mode(u8 mode)
{
	...
	ret = hp_wmi_perform_query(HPWMI_GRAPHICS_MUX_QUERY, HPWMI_WRITE, buffer, sizeof(buffer), sizeof(buffer));
	if (ret < 0)
		return ret;
	if (ret > 0)
		return -EINVAL;
...
static ssize_t gpu_mux_mode_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	u32 requested;
	u8 supported;
	int ret;
...
static void hp_wmi_notify(u32 value, void *context)
{
	...
	status = wmi_get_event_data(value, &buf);
...
static int __init hp_wmi_rfkill_setup(struct platform_device *device)
{
	...
	if (wireless & 0x1) {
		wifi_rfkill =
			rfkill_alloc("hp-wifi", &device->dev, RFKILL_TYPE_WLAN, &hp_wmi_rfkill_ops, (void *)HPWMI_WIFI);
		if (!wifi_rfkill)
			return -ENOMEM;
	...
	if (wireless & 0x2) {
		bluetooth_rfkill = rfkill_alloc("hp-bluetooth", &device->dev, RFKILL_TYPE_BLUETOOTH, &hp_wmi_rfkill_ops,
	...
	if (wireless & 0x4) {
		wwan_rfkill =
			rfkill_alloc("hp-wwan", &device->dev, RFKILL_TYPE_WWAN, &hp_wmi_rfkill_ops, (void *)HPWMI_WWAN);
		if (!wwan_rfkill) {
			err = -ENOMEM;
...
static int __init hp_wmi_rfkill2_setup(struct platform_device *device)
{
	...
	for (i = 0; i < state.count; i++) {
		...
		rfkill = rfkill_alloc(name, &device->dev, type, &hp_wmi_rfkill2_ops, (void *)(long)i);
		if (!rfkill) {
			err = -ENOMEM;
			goto fail;
		}
...
static bool has_omen_thermal_profile_ec_timer(void)
{
	...
	return match_string(omen_timed_thermal_profile_boards, ARRAY_SIZE(omen_timed_thermal_profile_boards),
			    board_name) >= 0;
}
...
{
	return hp_wmi_perform_query(HPWMI_THERMAL_PROFILE_QUERY, HPWMI_WRITE, &thermal_profile, sizeof(thermal_profile),
				    0);
}
...
static bool is_victus_thermal_profile(void)
{
	...
	return match_string(victus_thermal_profile_boards, ARRAY_SIZE(victus_thermal_profile_boards), board_name) >= 0;
}
...
static int platform_profile_victus_get(struct device *dev, enum platform_profile_option *profile)
{
...
static int victus_s_gpu_thermal_profile_get(bool *ctgp_enable, bool *ppab_enable, u8 *dstate, u8 *gpu_slowdown_temp)
{
	struct victus_gpu_power_modes gpu_power_modes;
	int ret;

	ret = hp_wmi_perform_query(HPWMI_GET_GPU_THERMAL_MODES_QUERY, HPWMI_GM, &gpu_power_modes,
				   sizeof(gpu_power_modes), sizeof(gpu_power_modes));
	if (ret == 0) {
...
static int victus_s_gpu_thermal_profile_set(bool ctgp_enable, bool ppab_enable, u8 dstate)
{
	...
	ret = victus_s_gpu_thermal_profile_get(&current_ctgp_state, &current_ppab_state, &current_dstate,
	...
	ret = hp_wmi_perform_query(HPWMI_SET_GPU_THERMAL_MODES_QUERY, HPWMI_GM, &gpu_power_modes,
				   sizeof(gpu_power_modes), 0);
...
static int victus_s_set_cpu_pl1_pl2(u8 pl1, u8 pl2)
{
	...
	ret = hp_wmi_perform_query(HPWMI_SET_POWER_LIMITS_QUERY, HPWMI_GM, &power_limits, sizeof(power_limits), 0);
...
static int platform_profile_victus_s_get_ec(enum platform_profile_option *profile)
{
	...
	if (params->ec_tp_offset == HP_EC_OFFSET_UNKNOWN || params->ec_tp_offset == HP_NO_THERMAL_PROFILE_OFFSET) {
		*profile = active_platform_profile;
		return 0;
	}
	 ...
	 * of these values.
	 */
	if (tp == victus_s_thermal_params.performance || tp == omen_v1_thermal_params.performance) {
		*profile = PLATFORM_PROFILE_PERFORMANCE;
	} else if (tp == victus_s_thermal_params.balanced || tp == omen_v1_thermal_params.balanced) {
		/*
...
static int platform_profile_victus_s_set_ec(enum platform_profile_option profile)
{
	...
	err = victus_s_gpu_thermal_profile_set(gpu_ctgp_enable, gpu_ppab_enable, gpu_dstate);
	if (err < 0) {
...
static int platform_profile_victus_s_set(struct device *dev, enum platform_profile_option profile)
{
	int err;
...
static int platform_profile_victus_set(struct device *dev, enum platform_profile_option profile)
{
	int err;
...
static int omen_powersource_event(struct notifier_block *nb, unsigned long value, void *data)
{
	 ...
	 * one.
	 */
	if (power_supply_is_system_supplied() <= 0 || active_platform_profile == actual_profile) {
...
static int victus_s_powersource_event(struct notifier_block *nb, unsigned long value, void *data)
{
	struct acpi_bus_event *event_entry = data;
	enum platform_profile_option actual_profile;
	int err;
...
static int hp_wmi_apply_fan_settings(struct hp_wmi_hwmon_priv *priv)
{
	...
	case PWM_MODE_MAX:
		...
		mod_delayed_work(system_dfl_wq, &priv->keep_alive_dwork,
				 msecs_to_jiffies(KEEP_ALIVE_DELAY_SECS * 1000));
		return 0;
...
static umode_t hp_wmi_hwmon_is_visible(const void *data, enum hwmon_sensor_types type, u32 attr, int channel)
{
	switch (type) {
	case hwmon_pwm:
		return 0644;
...
static int hp_wmi_hwmon_read(struct device *dev, enum hwmon_sensor_types type, u32 attr, int channel, long *val)
{
	struct hp_wmi_hwmon_priv *priv;
	int rpm, ret;
	u8 mode;
	...
	case hwmon_pwm:
		if (attr == hwmon_pwm_input) {
			if (hp_wmi_fan_control_supported()) {
				...
				if (rpm < 0)
					return rpm;
				*val = rpm_to_pwm(rpm / 100,
						  (channel == GPU_FAN) ? priv->gpu_max_rpm : priv->cpu_max_rpm);
			} else {
		...
		scoped_guard(mutex, &priv->lock) mode = priv->mode;
		switch (mode) {
		case PWM_MODE_MAX:
		case PWM_MODE_MANUAL:
		case PWM_MODE_AUTO:
...
static int hp_wmi_hwmon_write(struct device *dev, enum hwmon_sensor_types type, u32 attr, int channel, long val)
{
	struct hp_wmi_hwmon_priv *priv;
	int cpu_rpm, gpu_rpm;
	...
	case hwmon_pwm:
		if (attr == hwmon_pwm_input) {
			...
			if (hp_wmi_fan_control_supported()) {
				u8 max_rpm = (channel == GPU_FAN) ? priv->gpu_max_rpm : priv->cpu_max_rpm;
				int rpm = pwm_to_rpm(val, max_rpm);
				rpm = clamp_val(rpm, priv->min_rpm, max_rpm);
				if (channel == CPU_FAN)
					priv->cpu_pwm = rpm_to_pwm(rpm, priv->cpu_max_rpm);
				else if (channel == GPU_FAN)
					priv->gpu_pwm = rpm_to_pwm(rpm, priv->gpu_max_rpm);
				else
					priv->cpu_pwm = rpm_to_pwm(rpm, priv->cpu_max_rpm);
			} else {
				u8 clamped = clamp_val(val, 0, 255);
				if (channel == GPU_FAN)
					priv->gpu_pwm = clamped;
				else
					priv->cpu_pwm = clamped;
			}
		...
		case PWM_MODE_MANUAL:
			...
			if (hp_wmi_fan_control_supported()) {
				...
				if (cpu_rpm >= 0)
					priv->cpu_pwm = rpm_to_pwm(cpu_rpm / 100, priv->cpu_max_rpm);
				gpu_rpm = hp_wmi_get_active_fan_speed(GPU_FAN);
				if (gpu_rpm >= 0)
					priv->gpu_pwm = rpm_to_pwm(gpu_rpm / 100, priv->gpu_max_rpm);
			}
...
static int hp_wmi_setup_fan_settings(struct hp_wmi_hwmon_priv *priv)
{
	...
	u8 min_rpm, cpu_max_rpm, gpu_max_rpm;
	...
	/* Initialize safe defaults for all devices */
	priv->min_rpm = 0;
	priv->cpu_max_rpm = OMEN_CPU_MAX_RPM;
	priv->gpu_max_rpm = OMEN_GPU_MAX_RPM;
	priv->cpu_pwm = 128;
	priv->gpu_pwm = 128;

	/*
	 * Try to get a better max_rpm from hardware before attempting the
	 * Victus S fan table query. This covers boards that respond to the
	 * max-speed query but not to the full fan table query.
	 */
	ret = hp_wmi_perform_query(HPWMI_FAN_SPEED_MAX_GET_QUERY, HPWMI_GM, &max_val, sizeof(max_val), sizeof(max_val));
	if (!ret && max_val != 0) {
		pr_info("HP WMI: Hardware reported Max RPM: %d000\n", max_val);
		priv->cpu_max_rpm = max_val;
		priv->gpu_max_rpm = max_val;
	}
	...
	ret = hp_wmi_perform_query(HPWMI_VICTUS_S_GET_FAN_TABLE_QUERY, HPWMI_GM, &fan_data, 4, sizeof(fan_data));
	if (ret) {
		pr_info("HP WMI: Fan table query failed (%d), using cpu_max_rpm=%u, gpu_max_rpm=%u\n", ret,
			priv->cpu_max_rpm, priv->gpu_max_rpm);
		return 0; /* fall back to defaults already set above */
	}

	fan_table = (struct victus_s_fan_table *)fan_data;
	if (fan_table->header.num_fans == 0) {
		pr_info("HP WMI: Fan table reports 0 fans, using cpu_max_rpm=%u, gpu_max_rpm=%u\n", priv->cpu_max_rpm,
			priv->gpu_max_rpm);
		return 0;
	}
	...
	for (i = 0; i < num_entries; i++) {
		...
		if (cpu_rpm < min_rpm)
			min_rpm = cpu_rpm;
		if (cpu_rpm > cpu_max_rpm)
			cpu_max_rpm = cpu_rpm;
		if (gpu_rpm > gpu_max_rpm)
			gpu_max_rpm = gpu_rpm;
	}

	if (min_rpm == U8_MAX || (cpu_max_rpm == 0 && gpu_max_rpm == 0)) {
		pr_info("HP WMI: Fan table empty after parsing, using cpu_max_rpm=%u, gpu_max_rpm=%u\n",
			priv->cpu_max_rpm, priv->gpu_max_rpm);
		return 0;
	}
	...
	if (cpu_max_rpm > 0)
		priv->cpu_max_rpm = cpu_max_rpm;
	if (gpu_max_rpm > 0)
		priv->gpu_max_rpm = gpu_max_rpm;
...
static int hp_wmi_hwmon_init(void)
{
	...
	if (ret)
		return ret;
	hwmon = devm_hwmon_device_register_with_info(dev, "hp", priv, &chip_info, NULL);
...
static void __init setup_active_board_params(void)
{
	...
	if (id) {
		...
		if (params->ec_tp_offset == HP_EC_OFFSET_UNKNOWN) {
			pr_warn("Unknown EC layout for board %s. Thermal profile readback will be disabled. Please "
...
static int __init hp_wmi_init(void)
{
	...
	if (hp_wmi_perform_query(HPWMI_HARDWARE_QUERY, HPWMI_READ, &tmp, sizeof(tmp), sizeof(tmp)) ==
